...
public class Report_Controller implements Initializable {
    ...
    public static ObservableList<String> get_reports_list() {
        ...
        reports_List.add("Appt Types by Month");
        reports_List.add("Consultant Schedule");
        reports_List.add("Appts per Month");
        return reports_List;
    }
    ...
    public String rpt_Appt_Types() throws SQLException {
        ...
        try {
            ...
            while (result_set.next()) {
                report.append(result_set.getString("Month")).append("          ").append(result_set.getString("Amount")).append("   ").append(result_set.getString("type")).append("\n");
            }
    ...
    public String rpt_num_Appts() throws SQLException {
        ...
        try {
            ...
            ResultSet result_set = connection.createStatement().executeQuery(String.format("SELECT MONTHNAME(start) Month, YEAR(start) Year, COUNT(*) Total "
                                                                                         + "FROM appointment GROUP BY Month;"));
            
            while (result_set.next()) {
                String month = result_set.getString("Month");
                String year = result_set.getString("Year");
                String amount = result_set.getString("Total");
